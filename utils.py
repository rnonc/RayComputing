...
def coordRay(spatialVec,rays):
    ...
    coord_direction = torch.einsum('borc,Rc->bRor',rotation,rays)#(b,r,o,3)

    position = spatialVec[...,6:].unsqueeze(1).repeat(1,rays.shape[0],1,1)#(b,r,o,3)

    rays = rays.unsqueeze(1).unsqueeze(0).repeat(position.shape[0],1,position.shape[2],1)#(b,r,o,3)
